...
public interface OrderRepository extends JpaRepository<Order, Integer>{
	...
	@Query("SELECT new ua.model.view.OrderView(o.id, t.tableNumber, o.status, o.price) FROM Order o JOIN o.table t WHERE t.cafe.id=?1 AND o.status=?2")
	List<OrderView> findPaidViews(Integer id, Status status);
	
	@Query("SELECT new ua.model.view.OrderView(o.id, t.tableNumber, o.status, o.price) FROM Order o JOIN o.table t WHERE t.cafe.id=?1 AND o.status=?2 OR o.status=?3")
	List<OrderView> findUnpaidViews(Integer id, Status status1, Status status2);
